...
GGraphNode::GGraphNode(GNode *node, GDescriptorsetServiceProtocol *descriptorsetService, GVULKANDevice& vulkanDevice, GCommandServiceProtocol *commandService) : node(node), rts(GMatrix::identityMatrix()) {
...
void GGraphNode::updateRTS(GMatrix& newRTSMatrix, GVULKANDevice& vulkanDevice) {
    rts = newRTSMatrix;
    rtsBuffer.refreshBuffer(&rts, vulkanDevice.getLogicalDevice());
}
